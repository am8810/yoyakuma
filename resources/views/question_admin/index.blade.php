        ...
        <div class="member-top-body">
            ...
            <div class="menu">
              ...
              <ul id="links01">
                ...
                次に「日時・定員数」の設定画面になりますので、カレンダーを直接クリックして予約を受ける日時・定員数を設定し、毎日や毎週の繰り返し設定が必要な場合チェックを入れます。設定ができたら「登録」をクリックするとカレンダーに反映されます。（繰り返し設定は新規で登録を行う際にしか表示されず、既存のカレンダーの設定画面には表示されません。）「登録」をクリックして作成したカレンダーは、個別で編集・削除することができ、「全て削除」で一括で削除することもできます。<br>
                「日時・定員数」の設定が完了したら「ページ作成」をクリックすると予約ページ作成完了になります。<br>
              ...
              <ul id="links03">
                <li><span class="answer">Ａ</span>　 作成した予約ページは<a href="{{ url('reservepages') }}" target="_blank">予約ページ編集</a>にて、確認したい予約ページの「表示 <i class="icon-another-window_w"></i>」をクリックすると確認することができます。<br>
              ...
              <ul id="links04">
                ...
                既にあるカレンダーの設定画面には繰り返し設定の表示はされないのでご注意ください。繰り返し期間は1年先までとなり、1年先以降の設定は先の日にちのカレンダーを別途選択して行ってください。</li>
              </ul>
              ...
              <ul id="links05">
                ...
                2年先、3年先、それ以降と繰り返し設定を行いたい場合は、1年先以降のカレンダーを表示し、別途「日時・定員数」の設定を行い、繰り返しのチェックを入れて設定をしてください。<br>
              ...
              <ul id="links08">
                ...
                「日時・定員数」の設定カレンダーにて、営業開始から終了までの時間内で、例えば1時間ごとに何人の定員数にするかを決めて設定を行います。（何分ごとに設定を行うかは15分単位で自由に選択できます）お昼休憩等がある場合は、その時間だけ設定を入れないでください。<br>
              ...
              <ul id="links09">
                <li><span class="answer">Ａ</span>　 予約が入った際に管理者に送られる予約通知メールは、複数のメールアドレスで受信が可能です。メールアドレスを複数設定しておくことで、予約を複数人ですぐに把握することができます。<br>
              ...
              <ul id="links10">
                <li><span class="answer">Ａ</span>　 公開の場合は誰でも予約ページを閲覧することができ、非公開の場合はログイン状態の管理者しか予約ページを閲覧することができません。非公開の予約ページをログイン状態の管理者以外の方が閲覧すると、「※ページが存在しません。」という表示になり、予約を行うことはできません。<br>
              ...
              <ul id="links12">
                <li><span class="answer">Ａ</span>　 予約カレンダーの色は、予約ページ編集の「基本情報」をクリックし、「予約カレンダーの色」の四角い色をクリックし、色を変更した後に「色を決定」をクリックしてください。「色を決定」をクリックすると「現在選択している色は#000000です。」の部分も変更されます。色が変更されたら「更新」をクリックしてください。（新規で予約ページを作成する時も同様の作業になります。）<br>
                詳しくは<a href="{{ url('performance/create#calendar-color') }}" target="_blank">コチラ</a>をご覧ください。</li>
              </ul>
              ...
              <ul id="links15">
                <li><span class="answer">Ａ</span>　 予約ページを削除したい場合は、<a href="{{ url('reservepages') }}" target="_blank">予約ページ編集</a>で削除したい予約ページの「削除」をクリックしてください。「削除しますか」という表示がされるので「OK」をクリックすると、予約ページが削除されます。（※予約ページを削除した場合、予約情報、顧客情報も全て削除されます。）<br>
                予約ページに設定されている特定の日時を削除したい場合は、<a href="{{ url('reservepages') }}" target="_blank">予約ページ編集</a>で「日時・定員数」をクリックし、削除したい日時のカレンダーをクリックして「削除」をクリックすると削除されます。全ての日時を削除したい場合は「全て削除」をクリックし、「本当に全て削除しますか？」と表示れさるので再度「全て削除」をクリックすると全ての日時が削除されます。</li>
              </ul>
            ...
            <div class="menu">
              ...
              <label for="menu_bar-2_10">Ｑ10. 顧客情報はどうやって確認しますか？</label>
              <ul id="links-2_10">
                <li><span class="answer">Ａ</span>　 顧客情報はメニューの「顧客一覧」をクリックすると確認することができます。<br>
              ...
              <ul id="links-2_13">
                <li><span class="answer">Ａ</span>　 一時的に予約を停止したい場合は、<a href="{{ url('reservepages') }}" target="_blank">予約ページ編集</a>の「基本情報」をクリックし、「公開設定」を「非公開」にチェックを入れて「更新」をクリックしてください。<br>
                <img src="{{ asset('img/operation-image-13.jpg')}}" alt="一時的に予約を停止したい場合"><br>
                予約ページは非公開状態となり、ログイン状態の管理者のみが閲覧可能で、一般ユーザーには閲覧されないようになります。非公開状態の予約ページは、<a href="{{ url('reservepages') }}" target="_blank">予約ページ編集</a>で「非公開」のタブをクリックすると確認できます。<br>
              ...
              <ul id="links-2_14">
                <li><span class="answer">Ａ</span>　 「予約ページをどうやって作成・編集すればいいか分からない」「どのように予約ページを運用すればいいか分からない」といったお困りごとがありましたら、zoomによって画面共有をしていただき操作サポートをいたします。zoomサポートは<a href="{{ url('support') }}" target="_blank">コチラ</a>からご依頼ください。
                </li>
              </ul>
              ...
              <ul id="links-2_16">
                <li><span class="answer">Ａ</span>　 有料会員をやめたい場合は、基本情報の<a href="{{route('mypage.membership')}}" target="_blank">有料会員登録</a>にて「有料会員をやめる」をクリックすると、「本当に有料会員をやめますか？」と表示されますので、再度「有料会員をやめる」をクリックすると有料会員から無料会員になります。<br>
                アカウント自体を削除（退会）したい場合は、基本情報の<a href="{{route('mypage.edit')}}" target="_blank">会員情報の編集</a>にて「退会する」をクリックすると、「本当に退会しますか？」と表示されますので、再度「退会する」をクリックするとアカウントが削除されます。<br>
            ...
            <div class="menu">
              ...
              <ul id="links-3_02">
                <li><span class="answer">Ａ</span>　 新規会員登録フォームで入力した情報は、基本情報の<a href="{{route('mypage.edit')}}" target="_blank">会員情報の編集</a>で「編集」をクリックすると変更することができます。変更を行ったら「保存」をクリックしてください。<br>
              ...
              <ul id="links-3_03">
                <li><span class="answer">Ａ</span>　 予約ページに表示されている「提供者」は、「基本情報」の<a href="{{route('mypage.edit')}}" target="_blank">会員情報の編集</a>にある「企業名・屋号名・団体名」を編集して変更することができます。</li>
              </ul>
              
              <input type="checkbox" id="menu_bar-3_04" />
              <label for="menu_bar-3_04">Ｑ4. 予約完了ページと、予約完了のお知らせメールに記載されている、お問い合わせ先は変更できますか？</label>
              <ul id="links-3_04">
                <li><span class="answer">Ａ</span>　 予約完了ページと、予約完了のお知らせメールに記載されているお問い合わせ先は、会員情報の「企業名・屋号名・団体名」「電話番号」が表示されます。基本情報の<a href="{{route('mypage.edit')}}" target="_blank">会員情報の編集</a>で変更が可能です。</li>
              </ul>

              <input type="checkbox" id="menu_bar-3_05" />
              <label for="menu_bar-3_05">Ｑ5. 予約ページのページアドレス（URL）は変更できますか？</label>
              <ul id="links-3_05">
                <li><span class="answer">Ａ</span>　 予約ページのページアドレス（URL）は変更ができない仕様となっております。ご自身のページアドレスは、基本情報の<a href="{{route('mypage.edit')}}" target="_blank">会員情報の編集</a>で確認することができます。</li>
              </ul>
